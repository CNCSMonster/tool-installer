...
`binstall_first`, when present for `cargo-install`, must be a boolean and defaults to `false`. It is an install-time optimization for registry installs only. When true and the strategy is not a git install, tool-installer must ensure `cargo-binstall` is available by using an existing executable from `PATH` or `$CARGO_HOME/bin`/`~/.cargo/bin`, or by downloading the matching cargo-binstall release archive from `https://github.com/cargo-bins/cargo-binstall/releases/latest/download/` into the cargo bin directory. Bootstrap failures are non-fatal and must fall back to the normal `cargo install` command. The binstall attempt must run cargo-binstall with `-y --disable-strategies compile` unless `binstall_fallback_compile = true`, append `--version <selector>` for non-`latest` selectors, and fall back to the normal `cargo install` command if that attempt fails. `binstall_first` must not change installed-state check semantics.

`binstall_retry`, when present for `cargo-install`, must be a non-negative integer and defaults to `0`. It controls how many additional times the prebuilt binary bootstrap/download path is retried before falling back to the normal `cargo install` command. Retries apply to both the cargo-binstall bootstrap download and the cargo-binstall install attempt when `binstall_first` is enabled.

`binstall_fallback_compile`, when present for `cargo-install`, must be a boolean and defaults to `false`. When `true`, the binstall attempt omits `--disable-strategies compile`, allowing cargo-binstall to fall back to source compilation instead of failing immediately. When source compilation also fails, the failure is handled by the tool's failure policy (`allow_fail`).
